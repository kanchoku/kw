...
:(({softKeyboard=((|n|))}))
  ...
  ((|n|)) に 1 を指定すると、仮想鍵盤をソフトキーボードとして使えるようにします。
  仮想鍵盤でクリックされた位置のキーが押されたものとして扱います。

  ((%BS%))キー等は、仮想鍵盤の柱部分に割り当てています。
  上から、((%Esc%))、((%BS%))、ホットキー(ON/OFF トグル)、((%Shift%))。
  右下の空白のキーに((%Enter%))。最下段の余白部分に((%←%))、((%→%))。
  なお、縦長の枠表示の場合は、上から ((%Esc%))、((%BS%))、ホットキー(ON/OFF トグル)、((%>%))(語幹を縮める)、((%Enter%))。

  どの位置をクリックすればいいか参考用に、((*配字案内*)) が使えると思います。
  ((<(({defg=((|string|))}))>)) や ((<(({#define defguide "((|string|))"}))>))

  Windows 8.1で、漢直Winのウィンドウを移動すると、対象アプリケーションに
  文字の送付ができなくなる場合があります。一度漢直Winを終了してみてください。
...
:(({saveXYLoc=((|n|))}))
  ...
  ((|n|)) に 1 を指定すると、「漢直Win」終了時のウィンドウ位置を、
  kanchoku.ini に保存します。(({xLoc})) と (({yLoc})) の値を書き換えます。
